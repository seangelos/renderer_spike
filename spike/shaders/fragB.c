...
static vec4_t fragment_shader_phong(blinn_varyings_t* varyings,
                                    blinn_uniforms_t* uniforms,
                                    int* discard,
                                    int backface) {
    ...
    if (uniforms->punctual_intensity > 0) {
        ...
        if (n_dot_l > 0) {
            ...
            vec3_t refl_dir = vec3_sub(
                vec3_mul(material.normal, 2 * n_dot_l), 
                light_dir
            );
            float r_dot_v = vec3_dot(refl_dir, view_dir);
            vec3_t specular = vec3_new(0, 0, 0);
            if (!is_zero_vector(material.specular) && r_dot_v > 0) {
                specular = vec3_mul(
                    material.specular,
                    powf(r_dot_v, material.shininess)
                );
            }

            /* add terms into single punctual term */
...
vec4_t fragA(void *varyings_, void *uniforms_,
    ...
    if (uniforms->shadow_pass) {
        ...
        return fragment_shader_phong(varyings, uniforms, discard, backface);
    }
}
