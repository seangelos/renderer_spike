...
scene_t *blinn_triangle_scene(void) {
    mat4_t translation = mat4_translate(0.f, 0.f, 1.f);
    mat4_t rotation = mat4_rotate_x(TO_RADIANS(180));
    mat4_t root = mat4_mul_mat4(rotation, translation);
    return scene_from_file("triangle/triangle.scn", root);
}
